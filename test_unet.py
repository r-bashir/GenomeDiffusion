...
def test_unet():
    ...
    for i, t in enumerate(timesteps):
        ...
        if noise_pred.shape != noise.shape:
            print(f"WARNING: Shape mismatch at t={t}: noise_pred {noise_pred.shape}, noise {noise.shape}")
            if noise_pred.shape[2] != noise.shape[2]:
                noise_pred = F.interpolate(noise_pred, size=noise.shape[2], mode='linear')
        ...
        axs[i, 2].plot(noise_pred[0, 0].cpu().detach().numpy(), linewidth=1, label='Predicted', alpha=0.7)
    ...
    has_gradients = any(p.grad is not None and p.grad.abs().sum().item() > 0 for p in unet.parameters())
