...
def plot_ld_decay(real_samples, gen_samples, output_dir, max_distance=100, n_pairs=1000):
    ...
    plt.scatter(real_distances, real_r2, alpha=0.3, s=10, label="Real (raw)", color="blue")
    plt.scatter(gen_distances, gen_r2, alpha=0.3, s=10, label="Generated (raw)", color="orange")
    ...
    if not np.isnan(ld_corr):
        plt.text(
            0.95, 0.95, f"LD Pattern Correlation: {ld_corr:.4f}",
