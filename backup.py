...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels, widths, block_depth):
        ...
        for width in reversed(widths):
            self.up_blocks.append(UpBlock(width, out_channels if width == widths[0] else width, block_depth))
...
class SNPDiffusionProcess:
    ...
    def sample(self, x0: torch.Tensor, t: torch.Tensor, eps: torch.Tensor) -> torch.Tensor:
        ...
        alpha_t = self.alpha(t).view(-1, 1, 1, 1)  # Shape for broadcasting with 4D tensor
        sigma_t = self.sigma(t).view(-1, 1, 1, 1)
        
        # For valid values, apply diffusion; for missing values, keep them as is
        xt = torch.where(valid_mask, alpha_t * x0 + sigma_t * eps, torch.tensor(9.0, device=x0.device))
        
        return xt
    
    def predict_denoise(self, model_output: torch.Tensor, xt: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
...
class SNPDiffusionModel(nn.Module):
    def __init__(
        ...
        self.input_proj = nn.Conv2d(in_channels=1, out_channels=embedding_dim, kernel_size=1)
        
        # Output projection
        self.output_proj = nn.Conv2d(in_channels=embedding_dim, out_channels=1, kernel_size=1)
    ...
    def sample(self, shape, device, starting_noise=None):
        ...
        return self.diffusion.predict_denoise(pred_noise, x, torch.ones(shape[0], device=device))



# Example usage:
def train_snp_diffusion_model(snp_data, num_epochs=100, batch_size=32, learning_rate=1e-4):
    ...
    data_size = snp_data.shape[1] * snp_data.shape[2] if len(snp_data.shape) > 2 else snp_data.shape[1]
