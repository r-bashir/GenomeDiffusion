...
def weighted_mse_loss(predicted_noise, true_noise, t):
    ...
    """
    t = t.float()
    weights = 1.0 / (t + 1)  # +1 if t is 0-based
...
def plot_noise_evolution(
    timesteps, true_noises, pred_noises, snp_index, save_path=None
):
...
def track_single_run_at_snp(model, x0, snp_index):
...
def calculate_variance_across_runs(run_results):
...
def plot_variance_statistics(variance_stats, save_path=None):
